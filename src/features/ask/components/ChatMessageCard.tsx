...
import { Sparkles, Copy, RotateCcw, ThumbsUp, ThumbsDown, MoreHorizontal, Target, ChevronDown, ChevronUp, Check, Link, ShieldCheck, ArrowLeftRight } from 'lucide-react';
...
export function ChatMessageCard({ message, currentModel, onRegenerate, originalUserPrompt }: ChatMessageCardProps) {
        ...
        {message.isLoading ? (
          ...
          <>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-6 mt-4 border-t border-border/50">
              ...
              <Button onClick={() => navigate('/verify', { state: { messageId: message.id } })} variant="outline" size="sm" className="h-9 text-xs rounded-lg font-semibold border-border/60 shadow-xs hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 dark:hover:bg-indigo-900/20"><ShieldCheck className="w-3.5 h-3.5 mr-2" /> Verify</Button>
              <Button onClick={() => navigate(`/compare?prompt=${encodeURIComponent(originalUserPrompt || message.content)}`)} variant="outline" size="sm" className="h-9 text-xs rounded-lg font-semibold border-border/60 shadow-xs hover:text-orange-600 hover:border-orange-200 hover:bg-orange-50 dark:hover:bg-orange-900/20"><ArrowLeftRight className="w-3.5 h-3.5 mr-2" /> Compare</Button>
