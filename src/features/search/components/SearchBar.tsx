  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-xl z-50">
       ...
       {isFocused && (query.trim().length > 0) && (
         <div className="absolute top-full left-0 right-0 mt-2 bg-background border border-border/60 rounded-xl shadow-lg overflow-hidden flex flex-col max-h-100 overflow-y-auto animate-in fade-in slide-in-from-top-2">
           {error ? (
             ...
             <div className="py-2">
               {results.map((result) => (
                 <div key={result.id} className="flex items-center gap-3 px-3 py-2.5 mx-2 rounded-lg hover:bg-muted/50 cursor-pointer transition-colors">
                   <div className={['p-2 rounded-lg shrink-0', result.type === 'conversation' ? ['bg-blue-100', 'text-blue-600', 'dark:bg-blue-900/30'].join(' ') : ['bg-orange-100', 'text-orange-600', 'dark:bg-orange-900/30'].join(' ')].join(' ')}>
