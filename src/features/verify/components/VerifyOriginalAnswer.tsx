...
export function VerifyOriginalAnswer({
  ...
  const [localText, setLocalText] = useState(originalAnswer.text);
  const [editText, setEditText] = useState(originalAnswer.text);
    ...
    <div className="bg-card text-card-foreground shadow-sm border border-border/50 rounded-2xl shrink-0 flex flex-col p-4 md:p-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 md:mb-6 gap-3">
        ...
        <div className="flex items-center gap-3">
          <Badge variant="outline" className={['hidden md:flex px-3 py-1.5 text-[13px] font-bold gap-2 shadow-xs', 'bg-indigo-50/50', 'dark:bg-indigo-900/10', 'border-indigo-100', 'dark:border-indigo-800/30', 'text-indigo-700', 'dark:text-indigo-400', 'rounded-lg'].join(' ')}>
            <Sparkles className="w-3.5 h-3.5" /> {originalAnswer.model}
          </Badge>
          <Button onClick={() => setIsEditing(true)} variant="outline" size="sm" className="hidden md:flex h-9 px-4 gap-2 rounded-lg font-semibold text-[13px] border-border/60 hover:bg-muted/50 shadow-xs text-foreground">
          ...
          <div className="bg-card w-full max-w-4xl rounded-3xl shadow-2xl border border-border/50 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 px-6 border-b bg-muted/20">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <h3 className="font-bold text-[15px]">Edit Answer</h3>
                  <span className="text-xs text-muted-foreground font-medium">Modify the original generated text</span>
                </div>
              </div>
              <Button variant="ghost" size="icon" onClick={() => setIsEditing(false)} className="rounded-full bg-background border shadow-sm hover:bg-muted">
                <X className="w-5 h-5" />
              </Button>
            </div>
            ...
            <div className="p-6">
              <textarea
                ...
                className="w-full h-[40vh] p-4 text-[14px] leading-relaxed bg-background border border-border/60 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none font-medium"
